<template>
  <div class="relative">
    <ToolbarTop :t="t" :theme="theme" :current-lang="currentLang"
    ...
    <div class="solid-card p-6 shadow-xl mb-8">
      ...
      <CollageCanvas :t="t" :key="gridVersion" ref="canvasRef" class="mb-4"
...
/>
    ...
    <ExportPanel :t="t" :file-base="fileBase" :format="format" :loading="loading"
      @update:file-base="(v)=> fileBase = v" @update:format="(v)=> format = v" @export="downloadFile" />

    <HistoryPanel class="mt-8" :t="t" :history-list="history" @save="saveHistory" @load="loadHistory" @delete="deleteHistory" />
  </div>
</template>
...
import { translations } from './translations' // 翻译表
...
  normalizeGridShape()
  gridVersion.value++ // ✅ 触发 CollageCanvas 更新
}
...
  normalizeGridShape()
  gridVersion.value++ // ✅ 强制刷新 CollageCanvas 渲染
}
...
const currentLang = ref(
  (isClient && window.localStorage && window.localStorage.getItem('collage_lang')) ||
  (isClient && navigator.language && navigator.language.toLowerCase().startsWith('zh') ? 'zh' : 'en')
)
watch(currentLang, v => { try { isClient && window.localStorage?.setItem('collage_lang', v) } catch(e){} })


/* ---- 取词函数（支持 {i} 变量替换） ---- */
const t = (key, vars = {}) => {
  const rec = translations[key] || {}
  let s = rec[currentLang.value] ?? rec.en ?? rec.zh ?? key
...
const applyLangToDocument = () => {
  ...
  // 同步 <html lang>，有助于屏幕阅读器与搜索引擎
  document.documentElement.setAttribute('lang', currentLang.value === 'zh' ? 'zh-CN' : 'en')
}

onMounted(applyLangToDocument)
// 语言改变时更新
watch(currentLang, () => applyLangToDocument())
...
async function downloadFile() {
  ...
  try {
    ...
    const canvas = await html2canvas(wrapperEl, {
      ...
      useCORS: true,
      onclone: (doc) => {
	  // 1) 隐藏所有交互层：拖拽 / 删除 / 新增按钮
	  doc.querySelectorAll(
		'.drag-layer, .delete-layer, .add-layer, .handle'
	  ).forEach(el => {
		el.style.display = 'none';
		// 对某些浏览器更保险：也去掉 pointer 事件与可见性
		el.style.pointerEvents = 'none';
		el.style.visibility = 'hidden';
	  });

	  // 2) 去掉网格及其所有子元素的边框/描边/阴影（包括占位虚线）
	  doc.querySelectorAll('.image-grid, .image-grid *').forEach(el => {
		el.style.border = 'none';
		el.style.outline = 'none';
		el.style.boxShadow = 'none';
	  });

	  // 3) 你原有的处理保留（脚注输入框、占位提示、外层边框等）
...
const HISTORY_KEY = 'collage_history_v2'; // 新 key，避免跟旧版冲突
...
// ==== IndexedDB helpers (pure JS) ====
const DB_NAME = 'collage-history';
const STORE   = 'images';
...
// 保存：元数据进 localStorage；图片转存到 IndexedDB（Blob）
async function saveHistory(name) {
  const settings = {
    columns: columns.value, rows: rows.value,
    pageMargin: pageMargin.value, columnGap: columnGap.value, rowGap: rowGap.value,
    footerGap: footerGap.value, fontSize: fontSize.value,
    bgColor: bgColor.value, watermarkText: watermarkText.value
  };

  const id = Date.now();
  const meta = {
    id,
    name: (typeof name === 'string' && name.trim()) ? name.trim() : new Date().toLocaleString(),
    createdAt: id,
    version: 2,
    settings,
    footers: [...footers],
  ...
  for (let i = 0; i < cells.length; i++) {
    const src = cells[i]?.src || null;
    const key = `h:${id}:img:${i}`;
    try {
      if (typeof src === 'string' && (src.startsWith('data:') || src.startsWith('blob:'))) {
        const blob = await fetch(src).then(r => r.blob());
        await idbPut(key, blob);
      } else {
        await idbDel(key); // 没图就清理旧数据位
      }
    } catch (e) {
      console.warn('save image failed', i, e);
    }
  }

  const list = getHistory();
  list.unshift(meta);
  try { setHistory(list); renderHistoryList(); }
  catch (e) { alert(t('saveError')); }
}

// 读取：先应用元数据，再从 IndexedDB 逐格取回 Blob -> ObjectURL
async function loadHistory(id) {
  ...
  for (let i = 0; i < total; i++) {
    const blob = await idbGet(`h:${id}:img:${i}`);
    if (blob) {
      usedV2 = true;
      const url = URL.createObjectURL(blob);
      setCellFromSrc(i, url);
    } else {
      cells[i] = { src: null, naturalW: 0, naturalH: 0 };
    }
  }

  // 兼容旧版（v1）— localStorage 里直接有 images 的情况
  if (!usedV2 && Array.isArray(rec.images)) {
    for (let i = 0; i < total; i++) {
      const src = rec.images[i] || null;
      if (!src) cells[i] = { src: null, naturalW: 0, naturalH: 0 };
      else setCellFromSrc(i, src);
    }
  }

  // 脚注
  const savedFooters = Array.isArray(rec.footers) ? rec.footers : [];
  footers.splice(0, footers.length, ...savedFooters.slice(0, columns.value));
  while (footers.length < columns.value) footers.push('');
}

// 删除：删除元数据 + 清理 IndexedDB 里的 blob
async function deleteHistory(id) {
  ...
  const count = rec && rec.count ? rec.count : (columns?.value * rows?.value) || 0;
  for (let i = 0; i < count; i++) {
    try { await idbDel(`h:${id}:img:${i}`); } catch (e) {}
  }
  renderHistoryList();
}
